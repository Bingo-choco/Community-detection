...
object FindMaximalCliques_Advance {
   ...
   */
  def bronKerbosch2(R:Set[VertexId],P:Set[VertexId],X:Set[VertexId],map:Map[VertexId, Set[VertexId]]): Unit ={
    if(P.toList.length ==0 && X.toList.length ==0){
      ...
      for (v <- (P &~ Nu)) {
        ...
        bronKerbosch2(R+v, P.intersect(Nv), X.intersect(Nv), map)
        X += v
        P -= v
      }
    }
